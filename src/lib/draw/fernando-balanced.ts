...
export const fernandoBalanced: DrawGenerator = (athleteCount, heatSize, roundCount) => {
...
export function getChildAndPosition(seed: number, config: {
  ...
  let position = Math.floor(initialSeed / numberOfChildren)
  const jump = (position * config.roundIndex)
  position = (position + Math.round(config.roundIndex * contestantsPerChild / config.roundCount)) % contestantsPerChild
  
  const childPosition = childOrder[(initialSeed + jump) % childOrder.length]
...
// [0, 0, 0, 1, 1, 1]
export function roundSeedOrderNew(contestantsPerChild: number, numberOfChildren: number): number[] {
  return Array.from({ length: contestantsPerChild }, () =>
    Array.from({ length: numberOfChildren }, (_, i) => i))
    .flat()
}
