...
export class TournamentDraw {
    ...
    private generateRound(): Athlete[][] {
        ...
        for (const targetSize of heatSizes) {
            ...
            while (currentHeat.length < targetSize && availableAthletes.length > 0) {
                ...
                availableAthletes.splice(availableAthletes.findIndex(a => a.id === selected.id), 1);
            }
    ...
    public getFairnessMetrics(): FairnessMetrics {
        ...
        const variance = facings.reduce((sum, val) => sum + Math.pow(val - expectedMeetings, 2), 0) / facings.length;
        const theoreticalVarianceBound = 
            (this.roundsNeeded * this.k * (this.k - 1)) / 
            (v * (v - 1)) * 
            (1 - this.k / v);

        return {
            variance,
            maxFacings: Math.max(...facings),
            minFacings: Math.min(...facings),
